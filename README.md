...
装进 profile 并重启后，DSH 界面上会多出一个 **「体检」Tab**（与「荐股」同级），
以及会话头部的一个小徽章：**常显只有一个颜色圆点 + 「体检」二字**，
**鼠标移上去**才展开完整状态条：
...
- 真实参数细节（如"其中实测判定 20"）放在 Tab 面板里，不挤占状态条。
...
**点击状态条徽章或 Tab 里的「重新体检」按钮**会立刻重新枚举工具注册表并刷新结果。
...
5. **实测 render（可选，默认关）**：真调用一次 `render()` 能提高判定率，但它可能有副作用；
   本版本未默认启用
...
7. **实机状态（2026-09-19）**：**已在真实 DSH 中挂载成功**——web profile 以 `link:` 方式接入后，
   GUI 插件列表可见 `contract-check`，说明 bundle patch 解析正确、`apply()` 已执行。
   仍待补的两点：**首次体检的日志行未经核对**（DSH 输出在启动它的控制台窗口，未落盘）；
   `tools/change` 事件的作用域是否到达本插件**仍未验证**，因此插件额外做了兜底：
   **首次见到会话流量时补做一次体检**。
...
```bash
npm run build         # 构建客户端产物 lib/client.js（必须先跑，否则 GUI 不显示）
npm test              # 142 个用例：判定三态 / 实测 / 审计 / 词汇表 / 插件壳 / 源码扫描 / 打包检查 / 状态与路由 / 客户端产物 / CLI
